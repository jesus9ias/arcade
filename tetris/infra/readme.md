...
AWS CDK app that provisions the hosting infrastructure for the **tetris** game. It consumes `GameStack` from [`@arcade/infra`](../../infra) and is deployed manually from local (see the monorepo [`spec.md`](../../spec.md) → "Infra deployed from local").
...
Stack id: `TetrisStack` (set in [`bin/app.ts`](bin/app.ts)).
...
| Variable | Purpose |
|---|---|
| `SUBDOMAIN` | Subdomain prefix for the game (e.g. `tetris`) |
...
```bash
npm run typecheck --workspace tetris/infra
```
...
| Output | Use |
|---|---|
| `SiteUrl` | The deployed game URL; set it as `PUBLIC_SITE_URL` in `tetris/frontend/.env` |
| `BucketName` | GitHub secret `TETRIS_BUCKET_NAME` (frontend deploy) |
| `DistributionId` | GitHub secret `TETRIS_DISTRIBUTION_ID` (frontend deploy) |
...
```bash
npm run destroy --workspace tetris/infra
```
