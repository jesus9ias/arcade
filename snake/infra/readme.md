...
AWS CDK app that provisions the hosting infrastructure for the **snake** game. It consumes `GameStack` from [`@arcade/infra`](../../infra) and is deployed manually from local (see the monorepo [`spec.md`](../../spec.md) → "Infra deployed from local").
...
Stack id: `SnakeStack` (set in [`bin/app.ts`](bin/app.ts)).
...
| Variable | Purpose |
|---|---|
| `SUBDOMAIN` | Subdomain prefix for the game (e.g. `snake`) |
...
```bash
npm run typecheck --workspace snake/infra
```
...
| Output | Use |
|---|---|
| `SiteUrl` | The deployed game URL; set it as `PUBLIC_SITE_URL` in `snake/frontend/.env` |
| `BucketName` | GitHub secret `SNAKE_BUCKET_NAME` (frontend deploy) |
| `DistributionId` | GitHub secret `SNAKE_DISTRIBUTION_ID` (frontend deploy) |
...
```bash
npm run destroy --workspace snake/infra
```
