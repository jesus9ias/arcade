# space-explorer — infrastructure

AWS CDK app that provisions the hosting infrastructure for the **space-explorer** game. It consumes `GameStack` from [`@arcade/infra`](../../infra) and is deployed manually from local (see the monorepo [`spec.md`](../../spec.md) → "Infra deployed from local").
...
Stack id: `SpaceExplorerStack` (set in [`bin/app.ts`](bin/app.ts)).
...
| Variable | Purpose |
|---|---|
| `SUBDOMAIN` | Subdomain prefix for the game (e.g. `space-explorer`) |
...
```bash
npm run typecheck --workspace space-explorer/infra
```
...
| Output | Use |
|---|---|
| `SiteUrl` | The deployed game URL; set it as `PUBLIC_SITE_URL` in `space-explorer/frontend/.env` |
| `BucketName` | GitHub secret `SPACE_EXPLORER_BUCKET_NAME` (frontend deploy) |
| `DistributionId` | GitHub secret `SPACE_EXPLORER_DISTRIBUTION_ID` (frontend deploy) |
...
```bash
npm run destroy --workspace space-explorer/infra
```
