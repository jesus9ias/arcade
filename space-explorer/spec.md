...
interface SampleConfig {
  ...
  shape?: SampleShape;      // cosmetic marker shape; defaults to DEFAULT_SAMPLE_SHAPE
}
...
| `space_intro_seen` | `"true"` sentinel string | Set once the player starts the story intro; suppresses auto-replay on later visits |
...
// en.json — illustrative; es.json must mirror the exact key set with non-empty values
{
  ...
  "intro.title": "The adventure begins",
  "intro.start": "Begin",
  "intro.skip": "Skip",
  "intro.replay": "Replay intro",
  "intro.mute": "Mute music",
  "intro.unmute": "Unmute music",
  // intro.p1 … intro.p6 — the six story paragraphs (see Stage 6)
...
### Stage 6 — Phase 3: Story intro (in progress; requires separate authorization per sub-step)

**Scope:** A first-visit story sequence that sets up the game's premise, shown before the level select. It is a **UI overlay outside the pure FSM** (managed by the `useIntro` controller, like the controls overlay), gated by the `space_intro_seen` localStorage flag. No pure-engine or locked-test changes.

**Sub-steps (each its own authorized stage):**

1. ✅ **Done (2026-07-01) — functional base (no audio, no SVG).** `IntroSequence` component + `useIntro` controller. Start screen with a **Begin** button (the future audio-unlock gesture) and a corner **Skip** button; on Begin, the story auto-advances through the six paragraphs (`intro.p1`…`intro.p6`) at `INTRO_PARAGRAPH_DURATION_MS` each, then reveals the level select. A **Replay** control in the top nav (next to the language toggle, shown when not in a mission) reopens it. Persistence: the seen flag is written **only** when the player presses Begin — skipping does not persist it. New `constants/intro.ts` (all literals), new `space_intro_seen` storage key. localStorage read/write is controller-side in `useIntro` (mirroring `useRover`'s persistence ownership), so no pure `validate*` function or locked test is touched.
2. ✅ **Done (2026-07-01).** Decorative animated SVG backdrop behind the intro: a deterministic twinkling starfield (`INTRO_STAR_COUNT`), a ringed planet with a slow bob, a comet that periodically streaks, and the explorer's ship gliding across. New presentation-only `IntroBackdrop` component (aria-hidden), all motion CSS-driven and disabled under `prefers-reduced-motion`. Also a **two-stage exit transition**: on finish (or skip) the last paragraph fades out first, then the whole overlay dissolves (`INTRO_EXIT_DURATION_MS`) to reveal the level select underneath. Paragraph entrances are a soft ease-out transition whose style **varies per paragraph** (bottom → right → center → top → left, cycled) so the story doesn't always slide in from the same side; `center` zooms in from the middle. No engine/controller/test changes.
3. ✅ **Done (2026-07-01).** Background music wired: a looping `<audio>` (`INTRO_AUDIO_SRC` = `/audio/intro.mp3`, served from `frontend/public/audio/`) started by the **Begin** click — the user gesture that satisfies the browser autoplay-with-sound policy. A corner **mute** toggle (`intro.mute`/`intro.unmute`) is shown while the story plays, and the music fades out over `INTRO_EXIT_DURATION_MS` as the overlay dissolves. Missing-file/rejected `play()` is swallowed, so the intro works with or without the asset. The `.mp3` file itself is dropped into `public/audio/` separately (a `.gitkeep` holds the folder; `public/audio/CREDITS.md` records attribution). Track source: Pixabay — "Musical Atmo Sound" by Alien_I_Trust / Dimension X (id 240618, Pixabay Content License). No engine/controller/test changes; CSP needs none (same-origin media falls back to `default-src 'self'`).
4. **Multi-scene animation overhaul (phased).**
   - **Phase A ✅ Done (2026-07-01) — structure + ship.** Replaced the single `IntroBackdrop` with a **config-driven, N-scene system**: `INTRO_SCENES` (in `constants/intro.ts`) is an ordered timeline of `{ key, startParagraph }`; the `IntroScene` manager renders the active scene (last entry with `startParagraph ≤` the current paragraph, `-1` = start screen) and **cross-fades** on change (`INTRO_SCENE_FADE_MS`). Scenes live in `IntroScenes.tsx` (`SpaceScene`, `CityScene`, `DepartureScene`, `ApproachScene`) with shared `Stars`/`Planet` (relief) helpers, plus a reusable `IntroShip` modelled on the reference art (rounded rocket, central porthole, side pods, tail fins, **no exhaust flame**). Ship motion is **one-directional and slow** (never reverses). All four scene slots are wired and verified switching by paragraph (space → city → departure → approach). No engine/controller/test changes.
   - **Phase B (refinement pass) ✅ Done (2026-07-01).** Iterated on the scenes: intro title/paragraph pinned light with a dark halo so text stays legible over the ship/planets; the departure & approach ship drift made long + `linear` so it never appears to stop (whatever the paragraph duration); `CityScene` windows given a **varied count per building** (2–6, `cityWindows` grid) with a **staggered blackout** subset (the energy crisis) as night falls; and the star field reworked from a linear formula to a `fract(sin)` **hash so stars scatter without a visible pairing/lattice**. Begin button focused via ref so Enter/Space start the intro.
   - **Phase C (partial) ✅ Done (2026-07-01) — two polish items.** City windows now **flicker a couple of times before going dark** (`intro-window-out` keyframe) instead of a plain fade, and the star field gained **parallax**: stars are split into a far and a near layer (`intro-stars--far/--near`) that drift at different rates for subtle depth (gentle `alternate` float, disabled under `prefers-reduced-motion`).
   - **Possible future enhancements ⏳ (optional, not blocking).** Deferred nice-to-haves, kept as a backlog: sync the city dusk→night timing to the scene length; city antenna/aircraft red blinking lights; ground haze/smog; a moon rising over the city; Earth's night-side city lights + stronger atmospheric halo on departure; atmospheric glow, rings, or an orbiting moon on the approach world, with the ship gently rotating and the planet relief intensifying as it nears; an occasional shooting star; a soft vignette to focus the text.
...
| 2026-06-30 | Rover and sample markers redrawn as themed silhouettes (cosmetic) | The rover is rendered in `GameCanvas` as a classic descent-probe silhouette (a wide "mushroom" cap over a narrow core, two splayed legs with footpads, central nozzle) instead of a plain rectangle, all geometry derived from `ROVER_WIDTH`/`ROVER_HEIGHT` so the collision box is unchanged. Sample markers are drawn as irregular polygons instead of circles: a `SampleShape` catalog (`ROCK`, `CRYSTAL`, `SHARD`, `NUGGET`, `EMBER`, `POD`, `GLOB`) lives in `constants/samples.ts` as normalized vertex sets plus a per-shape natural colour (`SAMPLE_SHAPE_COLOR`), and each level assigns a thematic shape per sample via the new optional `SampleConfig.shape` (defaults to `DEFAULT_SAMPLE_SHAPE`). Each mineral now renders in its own colour; a buried sample keeps the purple `SAMPLE_SUBSURFACE_COLOR` "fire the laser here" signal until exposed, then shows its natural colour. A **collected** sample is no longer drawn at all (the grey collected marker — and `SAMPLE_COLOR`/`SAMPLE_COLLECTED_COLOR` — were removed). Still purely presentational: the render simply skips collected samples; no engine, state, controller, or test changes — `shape` is optional so the locked fixtures stay valid, and the collection rules themselves are untouched |
| 2026-07-01 | Intro ship drift is long + linear so it never appears to stop | With a longer per-paragraph duration the departure/approach ship finished its animation mid-scene and froze on its final frame, reading as an abrupt stop. The ship animations are now long (44s/46s) and `linear`, travelling off-screen, so the drift never completes (and never appears to halt) within a scene regardless of the configured paragraph duration. Still one-directional. CSS-only |
| 2026-07-01 | Intro text pinned light with a dark halo for legibility over the scenes | The light paragraph text was blending into the light ship where they overlapped. Rather than moving the ship, the intro title and paragraphs now use a fixed light colour (`#f4f7ff`) plus a dark `text-shadow` halo, so they stay readable over the ship, bright planets and open space — and, being theme-independent, over the always-dark scene art in light theme too. CSS-only |
| 2026-07-01 | Intro backdrop replaced by a config-driven N-scene system (Phase 3 sub-step 4A) | The single decorative backdrop became a timeline of full-screen scenes tied to the story beats: `INTRO_SCENES` lists `{ key, startParagraph }` and the `IntroScene` manager cross-fades to the last scene whose `startParagraph ≤` the current paragraph (`-1` = start screen). Four scenes (`SpaceScene` cosmos, `CityScene` energy-crisis skyline at dusk→night, `DepartureScene` leaving Earth, `ApproachScene` nearing a new world) share `Stars`/`Planet` (relief, clipped terminator) helpers and a reusable `IntroShip` based on the reference art (rounded rocket + porthole + pods + fins, **no flame** — nothing burns in space). Ship motion is one-directional and slow (holds its final frame, never reverses), and the arrival planet was moved to a clean corner to drop the old overlapping-disc look. Presentation-only (new components, config, CSS); no engine/state/test/CSP changes. Built as Phase A (structure); per-scene detail is deferred to follow-up phases |
| 2026-07-01 | Intro background music is gesture-started, mutable, and fades out (Phase 3 sub-step 3) | A looping `<audio>` (`INTRO_AUDIO_SRC`, served from `frontend/public/audio/`) is started inside the **Begin** click handler — that user gesture is what satisfies the browser autoplay-with-sound policy, which is exactly why the intro opens on a Begin screen rather than auto-playing. Audio lives in the `IntroSequence` component (a side effect, kept out of the pure layers) via an `audioRef`; a corner mute toggle flips `audio.muted`, and on exit the volume ramps to 0 over `INTRO_EXIT_DURATION_MS` while the overlay dissolves, with a pause on unmount. A missing file or a rejected `play()` is caught and ignored, so the story still runs before the `.mp3` is added (the file is committed separately into `public/audio/`; a `.gitkeep` holds the folder). No CSP change — same-origin media falls back to `default-src 'self'`. Presentation/controller-only; no engine, state, or test changes |
| 2026-07-01 | Intro paragraphs enter with a per-paragraph varied style and a soft ease-out | The story text no longer always slides up from the bottom. Each paragraph's entrance cycles bottom → right → center → top → left (`ENTER_DIRECTIONS` in `IntroSequence`, mapped to `.intro__paragraph--from-*` → `intro-enter-*` keyframes); `center` zooms in from `scale(0.9)` rather than sliding from an edge. Timing is a soft ease-out `cubic-bezier(0.16, 1, 0.3, 1)` over 1.3s with reduced offsets for a gentler feel. Verified by reading each paragraph's computed `animation-name` and sampling the transform mid-entrance (edge slides start offset ~±14–22px; the `center` paragraph starts at `scale ≈ 0.94`, settling to 1). Disabled under `prefers-reduced-motion`. Presentation-only |
