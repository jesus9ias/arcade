...
- **Phase 3 sub-step 1 (Stage 6, shipped 2026-07-01):** story intro — functional base (text only, no audio/SVG). New `IntroSequence` component + `useIntro` controller show a first-visit story overlay **outside the FSM** (like the controls overlay): a Begin start-screen, auto-advancing paragraphs (`intro.p1`…`intro.p6`), a corner Skip, and a top-nav Replay. Gated by the new `space_intro_seen` localStorage key; the flag is written only on Begin. localStorage read/write is controller-side in `useIntro`, so no pure/validation function or locked test changed. New `constants/intro.ts`.
- **Phase 3 sub-step 2 (Stage 6, shipped 2026-07-01):** decorative animated SVG backdrop for the intro — new presentation-only `IntroBackdrop` component (twinkling starfield, ringed planet, comet, gliding ship), all motion CSS-driven and disabled under `prefers-reduced-motion`. Plus a two-stage exit transition (`.intro--exiting`): last paragraph fades, then the overlay dissolves (`INTRO_EXIT_DURATION_MS`) to reveal the level select. Paragraph entrances use a soft ease-out with a per-paragraph varied style (bottom→right→center→top→left, `ENTER_DIRECTIONS`; `center` zooms from `scale(0.9)`). `INTRO_STAR_COUNT` in `constants/intro.ts`. No engine/controller/test changes.
- **Phase 3 sub-step 4A (Stage 6, shipped 2026-07-01):** multi-scene intro — replaced `IntroBackdrop` with a config-driven scene timeline. `INTRO_SCENES` (`constants/intro.ts`) maps paragraphs → scenes; `IntroScene` manager cross-fades (`INTRO_SCENE_FADE_MS`) between `SpaceScene`/`CityScene`/`DepartureScene`/`ApproachScene` (`IntroScenes.tsx`, shared `Stars`/`Planet` helpers) and a reusable `IntroShip` (reference-art rocket, no flame; one-directional slow motion). Per-scene visual detail is a pending follow-up (Phases B/C/D). No engine/controller/test changes.
- **Phase 3 sub-step 3 (Stage 6, shipped 2026-07-01):** intro background music — a looping `<audio>` (`INTRO_AUDIO_SRC` = `/audio/intro.mp3`, from `frontend/public/audio/`) started by the **Begin** gesture (satisfies autoplay-with-sound policy), a corner mute toggle (`intro.mute`/`intro.unmute`), and a volume fade-out on exit. Audio side effect lives in `IntroSequence` via `audioRef`; missing-file/rejected `play()` is swallowed. The `.mp3` is added into `public/audio/` separately (a `.gitkeep` holds the folder). No engine/controller/test/CSP changes.
