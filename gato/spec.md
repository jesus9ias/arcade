...
A meme is selected by picking a random file from the appropriate subfolder at `GAME_OVER` time. The catalog is read at runtime from a static JSON manifest (generated at build time or maintained manually by the developer) — not by scanning the filesystem at runtime.

```typescript
// src/lib/constants/memes.ts — populated by developer, not by Claude Code
// Shape expected by the meme loader:
interface MemeCatalog {
  win: string[];      // filenames under public/memes/win/
  lose: string[];     // filenames under public/memes/lose/
  neutral: string[];  // filenames under public/memes/neutral/
}
```
